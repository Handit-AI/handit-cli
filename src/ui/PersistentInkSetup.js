/**
 * Persistent Ink-based setup that doesn't clear the console
 * Uses a different approach to avoid clearing previous output
 */

/**
 * Show setup wizard without clearing console
 */
async function showPersistentSetupWizard(config) {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      function EntryFileStep({ onComplete, onBack, defaultFile = 'index.js' }) {
        ...
        return React.createElement(Box, { flexDirection: 'column', padding: 2 }, [
          // Step indicator
          React.createElement(Box, { key: 'step', marginBottom: 2 }, [
            React.createElement(Text, { key: 'step-text', color: 'yellow', bold: true }, '🎯 Step 2: Agent Entry Point'),
          ]),
...
module.exports = { showPersistentSetupWizard };
