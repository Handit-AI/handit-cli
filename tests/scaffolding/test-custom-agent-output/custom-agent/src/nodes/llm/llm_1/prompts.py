"""
Prompt definitions for the llm_1 LLM node
"""
...
def get_prompts() -> Dict[str, Any]:
    """
    Define prompts for the llm_1 LLM node.
    ...
    """
    return {
        "system": f"""You are a helpful AI assistant specialized in llm_1 tasks.

Your role is to:
- Process input data for the llm_1 stage
- Apply appropriate llm_1 logic using LLM capabilities
- Return structured results for the next stage

Guidelines:
- Be precise and focused on llm_1 functionality
- Maintain consistency with the overall agent workflow
- Handle errors gracefully
- Provide clear, actionable outputs""",

        "user_template": """Process the following input for the llm_1 LLM stage:
...
Please provide a structured response that can be used by the next stage in the pipeline.""",
        ...
        "examples": [
            {
                "input": f"Sample input for llm_1",
                "output": f"Sample LLM output from llm_1",
                "context": "Example context"
            }
        ],
        
        "error_handling": {
            "invalid_input": f"The input provided is not valid for llm_1 LLM processing.",
            "processing_error": f"An error occurred while processing the input in llm_1 LLM.",
            "timeout": f"The llm_1 LLM processing timed out."
        },
...
def get_node_specific_prompts(node_type: str) -> Dict[str, Any]:
    ...
    node_prompts = {
        "llm_1": {
            "description": "Customize this description for the llm_1 node functionality",
            "focus": "Define the specific focus and purpose of the llm_1 node",
