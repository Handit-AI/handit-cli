/**
 * Llm_1 LLM node processor
 */

import { BaseLLMNode } from '../../../base.js';
import { getPrompts } from './prompts.js';

export class Llm_1LLMNode extends BaseLLMNode {
  ...
  async process(inputData) {
    try {
      ...
      const result = await Promise.race([
        this._execute_llm_1_llm_logic(inputData),
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Timeout in llm_1 after 60 seconds')), 60000)
        )
      ]);
  ...
  async _execute_llm_1_llm_logic(inputData) {
