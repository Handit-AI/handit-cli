/**
 * Llm_2 LLM node processor
 */

import { BaseLLMNode } from '../../../base.js';
import { getPrompts } from './prompts.js';

export class Llm_2LLMNode extends BaseLLMNode {
  ...
  async process(inputData) {
    try {
      ...
      const result = await Promise.race([
        this._execute_llm_2_llm_logic(inputData),
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Timeout in llm_2 after 60 seconds')), 60000)
        )
      ]);
  ...
  async _execute_llm_2_llm_logic(inputData) {
